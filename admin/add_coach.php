...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        // Prevent adding duplicate coach numbers to the SAME train (e.g., two 'S1's on Train 12004)
        $check_query = "SELECT id FROM coaches WHERE train_id = ? AND coach_number = ?";
        $check_stmt = mysqli_prepare($conn, $check_query);
        mysqli_stmt_bind_param($check_stmt, "is", $train_id, $coach_number);
        mysqli_stmt_execute($check_stmt);
        mysqli_stmt_store_result($check_stmt);

        if (mysqli_stmt_num_rows($check_stmt) > 0) {
            $error = "Error: Coach '<strong>$coach_number</strong>' already exists on this specific train.";
        } else {
            $query = "INSERT INTO coaches (train_id, coach_type_id, coach_number, total_seats) VALUES (?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "iisi", $train_id, $coach_type_id, $coach_number, $total_seats);

            if (mysqli_stmt_execute($stmt)) {
                header("Location: manage_coaches.php?msg=added");
                exit();
            } else {
...
<head>
    ...
    <title>Add Coach | RailWayX Admin</title>
    ...
    <div class="flex max-w-[1800px] mx-auto relative flex-1 w-full overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-8 relative">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 relative z-10">
                <div>
                    <h2 class="text-3xl font-black text-white tracking-tight">Add New Coach 💺</h2>
                    <p class="text-sm text-slate-400 mt-1">Assign a new carriage to an existing train composition.</p>
                </div>
            ...
            <div class="bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-10 shadow-2xl relative z-10 max-w-4xl">
                ...
                <form action="add_coach.php" method="POST" class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="train_id" required class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                                ...
                                <?php foreach($trains as $train): ?>
                                    <option value="<?= htmlspecialchars($train['id']) ?>">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">Coach Number (e.g. S1, B2, A1)</label>
                            <input type="text" name="coach_number" required maxlength="10" placeholder="S1" class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 uppercase">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">Total Seats</label>
                            <input type="number" name="total_seats" required min="1" placeholder="72" class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                        </div>
                    </div>
