...
// 1. Fetch stations for dropdowns
try {
...
// 2. Fetch Existing Train Data
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $id = $_GET['id'];
    $fetch_query = "SELECT * FROM trains WHERE id = ?";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        $train_data = mysqli_fetch_assoc($result);
    } else {
        header("Location: manage_trains.php?error=notfound");
        exit();
    }
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if the NEW train number already exists for a DIFFERENT train
    $check_query = "SELECT id FROM trains WHERE train_number = ? AND id != ?";
    $check_stmt = mysqli_prepare($conn, $check_query);
    mysqli_stmt_bind_param($check_stmt, "si", $train_number, $id);
    mysqli_stmt_execute($check_stmt);
    mysqli_stmt_store_result($check_stmt);

    if (mysqli_stmt_num_rows($check_stmt) > 0) {
        $error = "Error: Train Number '<strong>$train_number</strong>' is already assigned to another train.";
    } else {
        $update_query = "UPDATE trains SET train_name=?, train_number=?, source_station=?, destination_station=?, departure_time=?, arrival_time=?, total_seats=?, fare_ac=?, fare_sleeper=?, fare_general=?, running_days=?, status=? WHERE id=?";
        $update_stmt = mysqli_prepare($conn, $update_query);
        mysqli_stmt_bind_param($update_stmt, "ssssssiiddssi", $train_name, $train_number, $source_station, $destination_station, $departure_time, $arrival_time, $total_seats, $fare_ac, $fare_sleeper, $fare_general, $running_days, $status, $id);

        if (mysqli_stmt_execute($update_stmt)) {
            header("Location: manage_trains.php?msg=updated");
            exit();
        } else {
...
<head>
    ...
    <title>Edit Train | RailWayX Admin</title>
    ...
    <div class="flex max-w-[1800px] mx-auto relative">
        ...
        <main class="flex-1 min-h-[calc(100vh-5rem)] p-4 md:p-8 relative">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 relative z-10">
                <div>
                    <h2 class="text-3xl font-black text-white tracking-tight flex items-center gap-3">Edit Train 🚄</h2>
                    <p class="text-sm text-slate-400 mt-1">Update locomotive scheduling and configuration details.</p>
                </div>
            ...
            <div class="bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-10 shadow-2xl relative z-10 max-w-5xl">
                ...
                <form action="edit_train.php?id=<?= $id ?>" method="POST" class="space-y-6">
                    <input type="hidden" name="id" value="<?= htmlspecialchars($train_data['id']) ?>">
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">Train Number</label>
                            <input type="text" name="train_number" value="<?= htmlspecialchars($train_data['train_number']) ?>" required maxlength="20" class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">Train Name</label>
                            <input type="text" name="train_name" value="<?= htmlspecialchars($train_data['train_name']) ?>" required maxlength="100" class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <select name="source_station" required class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                                ...
                                <?php foreach($stations as $station): ?>
                                    <option value="<?= htmlspecialchars($station['code']) ?>" <?= ($station['code'] == $train_data['source_station']) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($station['code']) ?> - <?= htmlspecialchars($station['name']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="space-y-2">
                            ...
                            <select name="destination_station" required class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                                ...
                                <?php foreach($stations as $station): ?>
                                    <option value="<?= htmlspecialchars($station['code']) ?>" <?= ($station['code'] == $train_data['destination_station']) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($station['code']) ?> - <?= htmlspecialchars($station['name']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="datetime-local" name="departure_time" value="<?= date('Y-m-d\TH:i', strtotime($train_data['departure_time'])) ?>" required class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">Arrival Time</label>
                            <input type="datetime-local" name="arrival_time" value="<?= date('Y-m-d\TH:i', strtotime($train_data['arrival_time'])) ?>" required class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="number" name="total_seats" value="<?= htmlspecialchars($train_data['total_seats']) ?>" required min="1" class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">AC Fare (₹)</label>
                            <input type="number" step="0.01" name="fare_ac" value="<?= htmlspecialchars($train_data['fare_ac']) ?>" placeholder="0.00" class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">Sleeper Fare (₹)</label>
                            <input type="number" step="0.01" name="fare_sleeper" value="<?= htmlspecialchars($train_data['fare_sleeper']) ?>" placeholder="0.00" class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">Gen Fare (₹)</label>
                            <input type="number" step="0.01" name="fare_general" value="<?= htmlspecialchars($train_data['fare_general']) ?>" placeholder="0.00" class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <select name="running_days" class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                                <option value="All" <?= ($train_data['running_days'] == 'All') ? 'selected' : '' ?>>All</option>
                                <option value="Weekdays" <?= ($train_data['running_days'] == 'Weekdays') ? 'selected' : '' ?>>Weekdays</option>
                                <option value="Weekends" <?= ($train_data['running_days'] == 'Weekends') ? 'selected' : '' ?>>Weekends</option>
                                <option value="Bi-Weekly" <?= ($train_data['running_days'] == 'Bi-Weekly') ? 'selected' : '' ?>>Bi-Weekly</option>
                                <option value="Weekly" <?= ($train_data['running_days'] == 'Weekly') ? 'selected' : '' ?>>Weekly</option>
                            </select>
                        </div>
                        <div class="space-y-2">
                            ...
                            <select name="status" class="w-full bg-slate-900 border border-white/10 text-white rounded-xl px-4 py-3.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">
                                <option value="Active" <?= ($train_data['status'] == 'Active') ? 'selected' : '' ?>>Active</option>
                                <option value="Delayed" <?= ($train_data['status'] == 'Delayed') ? 'selected' : '' ?>>Delayed</option>
                                <option value="Cancelled" <?= ($train_data['status'] == 'Cancelled') ? 'selected' : '' ?>>Cancelled</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="pt-6 border-t border-white/10">
                        <button type="submit" class="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold py-3.5 px-8 rounded-xl shadow-lg shadow-blue-500/20 transition-all flex items-center gap-2">
                            <span>🔄</span> Update Train Profile
                        </button>
                    </div>
                </form>
