...
// Safe database query block using JOINS to get Train Name and Coach Type Name
try {
    $query = "
        SELECT 
            c.id, 
        ...
        ORDER BY t.train_number ASC, c.coach_number ASC
    ";
    
    $result = mysqli_query($conn, $query);

    if ($result) {
        $coaches = mysqli_fetch_all($result, MYSQLI_ASSOC);
    } else {
        $db_error = mysqli_error($conn);
    }
} catch (Exception $e) {
    // Catches errors if the tables don't exist yet
...
$current_page = 'manage_coaches.php'; // Keep the Coaches menu highlighted
?>
...
<head>
    ...
    <title>Manage Coaches | RailWayX Admin</title>
    ...
    <div class="flex max-w-[1800px] mx-auto relative">
        ...
        <main class="flex-1 min-h-[calc(100vh-5rem)] p-4 md:p-8 relative">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 relative z-10">
                <div>
                    <h2 class="text-3xl font-black text-white tracking-tight flex items-center gap-3">Coach Inventory 💺</h2>
                    <p class="text-sm text-slate-400 mt-1">Manage individual carriages, seating capacity, and train assignments.</p>
                </div>
                <a href="add_coach.php" class="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-semibold text-sm py-3 px-6 rounded-xl shadow-lg shadow-blue-500/20 transition-all flex items-center gap-2">
                    <span>+</span> Add Coach
                </a>
            </div>
            ...
            <div class="bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl relative z-10">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <h3 class="text-lg font-bold text-white">Active Coaches</h3>
                    <div class="relative w-full sm:w-72">
                        <input type="text" placeholder="Search coach number or train..." class="w-full bg-slate-900 border border-white/10 text-sm text-slate-200 rounded-xl pl-11 pr-4 py-2.5 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all">
                    ...
                    <table class="w-full text-left border-collapse whitespace-nowrap">
                        <thead>
                            <tr class="border-b border-white/10">
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Coach No.</th>
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Class / Type</th>
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Assigned Train</th>
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Total Seats</th>
                            ...
                            <?php if (count($coaches) > 0): ?>
                                <?php foreach ($coaches as $coach): ?>
                                    <tr class="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                                        ...
                                        <td class="py-4 px-4">
                                            <span class="inline-flex items-center px-3 py-1 rounded-md text-xs font-black tracking-widest bg-indigo-500/20 text-indigo-400 border border-indigo-500/30">
                                                <?= htmlspecialchars($coach['coach_number'] ?? 'N/A') ?>
                                            </span>
                                        </td>
                                        ...
                                        <td class="py-4 px-4">
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-white/5 text-slate-300 border border-white/10">
                                                <?= htmlspecialchars($coach['type_name'] ?? 'Unknown Type') ?>
                                            </span>
                                        </td>
                                        
                                        <td class="py-4 px-4">
                                            <div class="flex flex-col">
                                                <span class="font-bold text-white"><?= htmlspecialchars($coach['train_name'] ?? 'Unassigned') ?></span>
                                                <span class="text-xs text-slate-400">Train #<?= htmlspecialchars($coach['train_number'] ?? '---') ?></span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="py-4 px-4">
                                            <div class="flex items-center gap-2 text-slate-300 font-semibold">
                                                <span>💺</span>
                                                <?= htmlspecialchars($coach['total_seats'] ?? '0') ?>
                                            </div>
                                        </td>
                                        
                                        <td class="py-4 px-4 text-right">
                                            <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <a href="edit_coach.php?id=<?= $coach['id'] ?>" class="w-8 h-8 flex items-center justify-center bg-white/5 hover:bg-blue-600/20 text-slate-400 hover:text-cyan-400 rounded-lg transition-colors" title="Edit">
                                                    ✏️
                                                </a>
                                                <a href="delete_coach.php?id=<?= $coach['id'] ?>" onclick="return confirm('Are you sure you want to remove this coach from the database?');" class="w-8 h-8 flex items-center justify-center bg-white/5 hover:bg-red-500/20 text-slate-400 hover:text-red-400 rounded-lg transition-colors" title="Delete">
                                                    🗑️
                                                </a>
                                            </div>
                                        </td>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="py-12 text-center text-slate-500">
                                        ...
                                        <p class="text-xs mt-1">Click "Add Coach" to start building your train compositions.</p>
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                ...
                <div class="flex items-center justify-between mt-6 pt-6 border-t border-white/10">
                    <p class="text-xs text-slate-500">Showing <?= count($coaches) ?> entries</p>
