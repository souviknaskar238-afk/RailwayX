...
// Safe database query block fetching from the dedicated payments table
try {
    $query = "
        SELECT 
            p.transaction_id, 
...
$current_page = 'manage_payments.php';
?>
...
<head>
    ...
    <title>Manage Payments | RailWayX Admin</title>
    ...
    <div class="flex max-w-[1800px] mx-auto relative flex-1 w-full overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-8 relative">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 relative z-10">
                <div>
                    <h2 class="text-3xl font-black text-white tracking-tight flex items-center gap-3">Payment Ledger 💳</h2>
                    <p class="text-sm text-slate-400 mt-1">Track financial transactions, revenue streams, and refund statuses.</p>
                </div>
            ...
            <div class="bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl relative z-10">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    ...
                    <div class="relative w-full sm:w-72">
                        <input type="text" placeholder="Search Txn ID or PNR..." class="w-full bg-slate-900 border border-white/10 text-sm text-slate-200 rounded-xl pl-11 pr-4 py-2.5 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all">
                    ...
                    <table class="w-full text-left border-collapse whitespace-nowrap">
                        <thead>
                            <tr class="border-b border-white/10">
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Transaction ID</th>
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Related PNR</th>
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Passenger</th>
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Method</th>
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Date & Time</th>
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest text-right">Amount</th>
                            ...
                            <?php if (count($payments) > 0): ?>
                                <?php foreach ($payments as $payment): ?>
                                    <tr class="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                                        ...
                                        <td class="py-4 px-4">
                                            <span class="font-mono text-xs font-bold bg-white/5 border border-white/10 px-2 py-1 rounded text-slate-300 tracking-wider">
                                        ...
                                        <td class="py-4 px-4">
                                            <span class="font-mono text-cyan-400 font-bold tracking-widest text-sm">
                                                <?= htmlspecialchars($payment['pnr_number'] ?? 'N/A') ?>
                                            </span>
                                        </td>
                                        
                                        <td class="py-4 px-4 text-white">
                                            <?= htmlspecialchars($payment['passenger_name'] ?? 'Guest User') ?>
                                        </td>
                                        ...
                                        <td class="py-4 px-4">
                                            <span class="inline-flex items-center gap-2 text-slate-300">
                                                <?php 
                                                    ...
                                                    if (strpos($method, 'card') !== false) echo "💳";
                                                    elseif (strpos($method, 'upi') !== false) echo "📱";
                                                    elseif (strpos($method, 'net') !== false) echo "🏦";
                                                    else echo "💵";
                                                ?>
                                                <?= htmlspecialchars($payment['payment_method'] ?? 'Unknown') ?>
                                            </span>
                                        </td>

                                        <td class="py-4 px-4 text-slate-400 text-xs">
                                            <?= !empty($payment['transaction_date']) ? date('M d, Y h:i A', strtotime($payment['transaction_date'])) : '---' ?>
                                        </td>

                                        <td class="py-4 px-4 text-right font-medium text-emerald-400">
                                        ...
                                        <td class="py-4 px-4 text-right">
                                            <?php 
                                                ...
                                                } elseif (strcasecmp($status, 'Refunded') == 0) {
                                                    $statusClass = 'text-slate-400 border-slate-500/30 bg-slate-500/10';
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="7" class="py-16 text-center text-slate-500">
                                        ...
                                        <p class="text-base font-semibold">No payment records found.</p>
                                        <p class="text-xs mt-1">Processed transactions will appear here automatically.</p>
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                ...
                <div class="flex items-center justify-between mt-6 pt-6 border-t border-white/10">
                    <p class="text-xs text-slate-500">Showing <?= count($payments) ?> entries</p>
