...
// Fetch all stations using procedural MySQLi
$stations = [];
$query = "SELECT id, name, code, city, state, address FROM stations ORDER BY name ASC";
...
<head>
    ...
    <title>Manage Stations | RailWayX Admin</title>
    ...
    <div class="flex max-w-[1800px] mx-auto relative">
        ...
        <main class="flex-1 min-h-[calc(100vh-5rem)] p-4 md:p-8 relative">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 relative z-10">
                <div>
                    <h2 class="text-3xl font-black text-white tracking-tight">Station Network</h2>
                    <p class="text-sm text-slate-400 mt-1">Manage railway stations, transit hubs, and route nodes.</p>
                </div>
                <a href="add_station.php" class="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-semibold text-sm py-3 px-6 rounded-xl shadow-lg shadow-blue-500/20 transition-all flex items-center gap-2">
            ...
            <div class="bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl relative z-10">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <h3 class="text-lg font-bold text-white">Active Stations</h3>
                    <div class="relative w-full sm:w-72">
                        <input type="text" placeholder="Search by name or code..." class="w-full bg-slate-900 border border-white/10 text-sm text-slate-200 rounded-xl pl-11 pr-4 py-2.5 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all">
                    ...
                    <table class="w-full text-left border-collapse whitespace-nowrap">
                        <thead>
                            <tr class="border-b border-white/10">
                                ...
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Station Code</th>
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Station Name</th>
                                <th class="pb-4 px-4 text-xs font-semibold text-slate-400 uppercase tracking-widest">Location</th>
                            ...
                            <?php if (count($stations) > 0): ?>
                                <?php foreach ($stations as $station): ?>
                                    <tr class="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                                        <td class="py-4 px-4 text-slate-500 font-mono text-xs">#<?= htmlspecialchars($station['id']) ?></td>
                                        
                                        <td class="py-4 px-4">
                                            <span class="inline-flex items-center px-3 py-1 rounded-md text-xs font-black tracking-wider bg-slate-900 text-cyan-400 border border-white/10">
                                                <?= htmlspecialchars($station['code']) ?>
                                            </span>
                                        </td>
                                        ...
                                        <td class="py-4 px-4">
                                            <div class="flex flex-col gap-1 text-xs">
                                                <span class="text-slate-300">🏙️ <?= htmlspecialchars($station['city']) ?></span>
                                                <span class="text-slate-400">📍 <?= htmlspecialchars($station['state']) ?></span>
                                            </div>
                                        </td>
                                        
                                        <td class="py-4 px-4 text-right">
                                            <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <a href="edit_station.php?id=<?= $station['id'] ?>" class="w-8 h-8 flex items-center justify-center bg-white/5 hover:bg-blue-600/20 text-slate-400 hover:text-cyan-400 rounded-lg transition-colors" title="Edit">
                                                    ✏️
                                                </a>
                                                
                                                <a href="delete_station.php?id=<?= $station['id'] ?>" onclick="return confirm('Are you sure you want to delete this station? Note: This may break trains assigned to this route.');" class="w-8 h-8 flex items-center justify-center bg-white/5 hover:bg-red-500/20 text-slate-400 hover:text-red-400 rounded-lg transition-colors" title="Delete">
                                                    🗑️
                                                </a>
                                            </div>
                                        </td>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="py-12 text-center text-slate-500">
                                        ...
                                        <p class="text-xs mt-1">Click "Add Station" to map your network.</p>
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                ...
                <div class="flex items-center justify-between mt-6 pt-6 border-t border-white/10">
                    <p class="text-xs text-slate-500">Showing <?= count($stations) ?> entries</p>
